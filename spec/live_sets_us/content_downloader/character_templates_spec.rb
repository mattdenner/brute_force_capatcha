...
describe LiveSetsUS::ContentDownloader::CharacterTemplates do
  ...
  describe '#correlate_with' do
    after(:each) do
      ...
      template_image_a.should_receive(:correlate_with).with(:image).and_return(@score_a)

      template_image_b = mock('template image b')
      template_image_b.should_receive(:correlate_with).with(:image).and_return(@score_b)

      described_class.new do |m,c|
        m[ 'a' ] = template_image_a
        m[ 'b' ] = template_image_b
      end.correlate_with(:image).should == @expected
    end
    ...
    it 'should order the results in descending correlation' do
      @score_a, @score_b = 10, 20
      @expected = [ [ 'b', 20 ], [ 'a', 10 ] ]
    end
  end
end
